...
#define UM_TRIG_PIN IOPORT_CREATE_PIN(PORTA, 7)
...
static void dma_init(void) {
	...
	dma_channel_set_source_address(&dmach_conf, (uint16_t)&ADCA.CH0RES);	
	dma_channel_set_dest_dir_mode(&dmach_conf, DMA_CH_DESTDIR_INC_gc);
	dma_channel_set_destination_address(&dmach_conf, (uint16_t)Samples);
...
int main (void) {
	...
	ioport_set_pin_dir(UM_TRIG_PIN, IOPORT_DIR_OUTPUT);
	...
	while(1) {
		...
		ioport_set_pin_level(UM_TRIG_PIN, true);
		ioport_toggle_pin_level(LED_PIN);
		
		delay_ms(1);
		
		ioport_set_pin_level(UM_TRIG_PIN, false);
		ioport_toggle_pin_level(LED_PIN);
